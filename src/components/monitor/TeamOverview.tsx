...
export const SessionOverview: React.FC<SessionOverviewProps> = ({
  ...
  // 优先使用 metrics 中的实时数据，如果不可用则回退到 session 数据
  const agentCount = metrics?.total_agents ?? session.agentCount;
  const activeAgentCount = metrics?.active_agents ?? session.activeAgents;
  const totalTasks = metrics?.total_tasks ?? session.totalTasks;
  const completedTasks = metrics?.completed_tasks ?? session.completedTasks;
    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          <span className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-green-100 text-green-800">
            {activeAgentCount} / {agentCount} 在线
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-4 mt-4">
        <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
          <div className="text-2xl font-bold text-gray-900 dark:text-white">{agentCount}</div>
