...
export async function startAttentionLoop(
  ...
  const checkLoop = async () => {
    while (isRunning) {
      ...
      // 周期性自检：当 Lead 持续空闲时，检查是否有未完成的任务
        ...
        if (idleTickCounter >= SELF_CHECK_INTERVAL_TICKS && !selfCheckPending) {
          ...
          try {
            ...
            if (pendingTasks.length > 0) {
              const unassigned = pendingTasks.filter(t => !t.assigneeId)
              const inProgress = pendingTasks.filter(t => t.status === 'IN_PROGRESS' || t.status === 'ASSIGNED')

              let alertContent = `[周期性任务自检] `
              if (unassigned.length > 0) {
                alertContent += `你有 ${unassigned.length} 个 PENDING 未分配任务需要处理。`
              }
              if (inProgress.length > 0) {
                alertContent += `${inProgress.length} 个任务正在执行中。`
              }
              alertContent += `\n请检查任务列表并采取适当行动。`

              console.log(
                `[AttentionManager][${agentId}] Periodic self-check: ${pendingTasks.length} incomplete tasks (${unassigned.length} unassigned)`
              )
              ...
              await deliverMessage(swarmSessionId, agentId, {
                ...
                priority: unassigned.length > 0 ? 'high' : 'low',
                metadata: {
                  autoGenerated: true,
