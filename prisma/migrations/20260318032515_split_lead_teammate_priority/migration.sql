/*
  ...
  - You are about to drop the column `priority` on the `llm_api_configs` table. All the data in the column will be lost.
  - Made the column `base_url` on table `llm_api_configs` required. This step will fail if there are existing NULL values in that column.
...
INSERT INTO "new_llm_api_configs" ("api_key", "base_url", "created_at", "default_model", "id", "is_enabled", "last_used_at", "max_context_tokens", "max_output_tokens", "name", "provider", "temperature", "updated_at", "user_id") SELECT "api_key", "base_url", "created_at", "default_model", "id", "is_enabled", "last_used_at", "max_context_tokens", "max_output_tokens", "name", "provider", "temperature", "updated_at", "user_id" FROM "llm_api_configs";
