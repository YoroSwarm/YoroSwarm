...
```
python3 _skills/pdf/scripts/pdf.py <command> <subcommand> [options]
```
...
```bash
python3 _skills/pdf/scripts/pdf.py form info input.pdf
```
...
**Step 2: Fill form**
```bash
python3 _skills/pdf/scripts/pdf.py form fill input.pdf -o output.pdf -d '{"name": "John", "agree": "true", "country": "US"}'
```
...
**Extract text**:
```bash
python3 _skills/pdf/scripts/pdf.py extract text document.pdf
python3 _skills/pdf/scripts/pdf.py extract text document.pdf -p 1-3    # Pages 1-3 only
python3 _skills/pdf/scripts/pdf.py extract text document.pdf -p 1,3,5  # Specific pages
```

**Extract tables**:
```bash
python3 _skills/pdf/scripts/pdf.py extract table document.pdf
```
...
**Merge PDFs**:
```bash
python3 _skills/pdf/scripts/pdf.py pages merge a.pdf b.pdf c.pdf -o merged.pdf
```

**Split PDF**:
```bash
python3 _skills/pdf/scripts/pdf.py pages split document.pdf -o ./output_dir/
```

**Rotate pages**:
```bash
python3 _skills/pdf/scripts/pdf.py pages rotate document.pdf 90 -o rotated.pdf
python3 _skills/pdf/scripts/pdf.py pages rotate document.pdf 180 -o rotated.pdf -p 1-3  # Specific pages
```

**Crop pages**:
```bash
python3 _skills/pdf/scripts/pdf.py pages crop document.pdf 50,50,550,750 -o cropped.pdf
```
...
**Read metadata**:
```bash
python3 _skills/pdf/scripts/pdf.py meta get document.pdf
```

**Set metadata**:
```bash
python3 _skills/pdf/scripts/pdf.py meta set document.pdf -o output.pdf -d '{"Title": "My Document", "Author": "John Doe"}'
```
...
```bash
python3 _skills/pdf/scripts/pdf.py convert input.docx -o output.pdf
```
