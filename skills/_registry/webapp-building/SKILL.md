...
1. `scripts/init-webapp.sh <website-title> [template-name]` — Initialize project in ./app
...
4. Deploy the build output in `./app/dist/`
...
```bash
# init project in ./app with website title
bash scripts/init-webapp.sh <website-title> [template-name]
cd ./app
```

**AI Agent Notes**:
- The project path is ./app
...
Deploy the build output in `./app/dist/`
